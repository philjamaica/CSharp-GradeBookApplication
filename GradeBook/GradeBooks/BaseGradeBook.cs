...
namespace GradeBook.GradeBooks
{
    public abstract class BaseGradeBook
    {
        ...
        {
            int weightedAdjustment = 0;
            if (IsWeighted && (studentType == StudentType.DualEnrolled || studentType == StudentType.Honors))
            {
                weightedAdjustment = 1;
            }

            switch (letterGrade)
            {
                case 'A':
                    return 4 + weightedAdjustment;
                case 'B':
                    return 3 + weightedAdjustment;
                case 'C':
                    return 2 + weightedAdjustment;
                case 'D':
                    return 1 + weightedAdjustment;
                case 'F':
                    return 0 + weightedAdjustment;
            }
            return 0;
        }
